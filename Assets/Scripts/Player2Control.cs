...
public class Player2Control : MonoBehaviour {
	...
	public string HorizontalCtrlPAD1 = "Horizontal_PAD1";
	public string HorizontalCtrlPAD2 = "Horizontal_PAD2";
	...
	void Start () {
		...
		selectedController = Constants.PAD_CONTROL_PIONE;
	}
	...
	void Update()
	{
        ...
        {
            posP2.x = widthOrtho - playerBoundaryRadius;
        }
        if (posP2.x - playerBoundaryRadius < -widthOrtho)
        {
            posP2.x = -widthOrtho + playerBoundaryRadius;
            ...
            case Constants.PAD_CONTROL_PIONE:
                ...
                if (Input.GetButton("Fire1_PAD1") && cooldownTimerFire1 <= 0)
                {
                    if (_currentAnimationState != STATE_DEAD && _currentAnimationState != STATE_DUCK && _currentAnimationState != STATE_TGRAN && _currentAnimationState != STATE_HGRAN && _currentAnimationState != STATE_WIN)
                    {
                        ...
                        if (goingRight)
                        {
                            newposP2 = posP2.x - 0.25f;
                        }
                        else if (!goingRight)
                        {
                            newposP2 = posP2.x + 0.25f;
                        }
                        Instantiate(bulletPrefabP2, new Vector2(newposP2, -3.0f), GetComponent<Transform>().rotation);
                        rifleFire = Instantiate(rifleFirePrefab, new Vector2(newposP2, -3.2f), GetComponent<Transform>().rotation) as GameObject;
                    }
                }
                if (Input.GetButtonDown("Fire2_PAD1"))
                {
                    ...
                    {
                        crosshair = Instantiate(crosshairPrefab, new Vector2(posP2.x, -2.5f), GetComponent<Transform>().rotation) as GameObject;
                ...
                if (Input.GetButtonUp("Fire2_PAD1"))
                {
                    ...
                    if (goingRight)
                    {
                        granade = Instantiate(granadePrefab, new Vector2(posP2.x + 0.5f, -3.0f), GetComponent<Transform>().rotation) as GameObject;
                        granadeStartPos = new Vector3(posP2.x + 0.5f, -3.0f, 0);
                    }
                    else
                    {
                        granade = Instantiate(granadePrefab, new Vector2(posP2.x - 0.5f, -3.0f), GetComponent<Transform>().rotation) as GameObject;
	...
	void FixedUpdate () {
		...
		case Constants.PAD_CONTROL_PIONE:
                ...
                {
                    if (crosshair.GetComponent<Player2Crosshair>().getPosition().x > this.GetComponent<Rigidbody2D>().position.x)
                    {
                        goingRight = false;
                    }
